...
pub fn socket_path_from(runtime_dir: Option<&str>, uid: u32, run_user_exists: bool) -> PathBuf {
    match runtime_dir.filter(|dir| Path::new(dir).is_absolute()) {
        ...
        None if run_user_exists => PathBuf::from(format!("/run/user/{uid}"))
            .join(SOCKET_DIR)
            .join(SOCKET_FILE),
...
mod tests {
    ...
    fn a_relative_or_empty_runtime_directory_is_ignored() {
        assert_eq!(
            socket_path_from(Some("relative"), 1000, true),
            PathBuf::from("/run/user/1000/phonia/phoniad.sock")
        );
        assert_eq!(
            socket_path_from(Some(""), 1000, false),
            PathBuf::from("/tmp/phonia-1000/phoniad.sock")
        );
    }
    ...
    fn without_a_runtime_directory_it_falls_back_per_user() {
        assert_eq!(
            socket_path_from(None, 1000, true),
            PathBuf::from("/run/user/1000/phonia/phoniad.sock")
        );
        assert_eq!(
            socket_path_from(None, 1000, false),
            PathBuf::from("/tmp/phonia-1000/phoniad.sock")
        );
        assert_ne!(
            socket_path_from(None, 1001, false),
            socket_path_from(None, 1000, false),
            "one per user"
        );
    }
    ...
    fn this_process_has_a_user_id_and_a_path() {
        ...
        assert!(
            path.is_absolute() && path.ends_with("phoniad.sock"),
            "{path:?}"
        );
    }
}
