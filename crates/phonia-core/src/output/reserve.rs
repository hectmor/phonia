...
pub enum ReserveError {
    ...
    Refused { device: String, holder: String, priority: Option<i32> },
    /// The holder did not answer in time.
    Unresponsive { device: String, holder: String },
    Failed(String),
}
...
impl fmt::Display for ReserveError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ReserveError::Refused { device, holder, priority } => {
            ...
            ReserveError::Unresponsive { device, holder } => {
                write!(f, "{holder} did not answer the request to release {device} in time")
            }
...
impl ReservationSlot {
    pub fn new(reserver: Arc<dyn DeviceReserver>) -> Self {
        Self { reserver, held: Mutex::new(None) }
    }
    ...
    pub fn ensure(&self, card: u32, device_name: &str) -> std::result::Result<Ensured, ReserveError> {
        let mut held = self.held.lock().unwrap();
        if held.as_ref().is_some_and(|(held_card, _)| *held_card == card) {
...
mod tests {
    ...
    fn busy() -> anyhow::Error {
        DeviceBusy { device: "hw:2,0".into(), detail: "EBUSY".into() }.into()
    }
    ...
    fn the_same_card_is_reserved_only_once() {
        ...
        assert_eq!(slot.ensure(2, "hw:DS2,0").unwrap(), Ensured::Acquired { took_over: false });
    ...
    fn a_refusal_keeps_nothing_held_and_can_be_retried() {
        let reserver = FakeReserver::new();
        reserver.script(Err(ReserveError::Refused { device: "hw:DS2,0".into(), holder: "jackd".into(), priority: Some(99) }));
        let slot = ReservationSlot::new(reserver.clone());
        let error = slot.ensure(2, "hw:DS2,0").unwrap_err().to_string();
        assert!(error.contains("jackd") && error.contains("reserve = false"), "{error}");
        assert_eq!(slot.held_card(), None);
        assert!(slot.ensure(2, "hw:DS2,0").is_ok(), "the next attempt is granted");
    }
    ...
    fn a_refusal_stops_the_open() {
        let reserver = FakeReserver::new();
        reserver.script(Err(ReserveError::Unresponsive { device: "hw:2,0".into(), holder: "jackd".into() }));
        ...
        assert!(!opened.get(), "the device must not be opened after a failed reservation");
    }
    ...
    fn without_a_bus_the_open_goes_ahead() {
        ...
        let slot = ReservationSlot::new(reserver);
        assert_eq!(open_reserved(Some(&slot), Some((2, "hw:2,0")), || Ok(1)).unwrap(), 1);
    }
    ...
    fn without_a_bus_a_busy_device_explains_why() {
        ...
        reserver.script(Err(ReserveError::NoBus("no session".into())));
        let slot = ReservationSlot::new(reserver);
        let error = open_reserved(Some(&slot), Some((2, "hw:2,0")), || -> Result<()> { Err(busy()) }).unwrap_err();
        assert!(format!("{error:#}").contains("no D-Bus session"), "{error:#}");
    }
    ...
    fn a_busy_device_is_retried_only_after_taking_the_card_from_someone() {
        ...
        let slot = ReservationSlot::new(reserver);
        open_reserved_with(Some(&slot), Some((2, "hw:2,0")), 5, Duration::ZERO, &mut || flaky(&attempts)).unwrap();
        assert_eq!(attempts.get(), 3, "took over: retried until the holder let go");

        attempts.set(0);
        let reserver = FakeReserver::new();
        let slot = ReservationSlot::new(reserver);
        let error = open_reserved_with(Some(&slot), Some((2, "hw:2,0")), 5, Duration::ZERO, &mut || flaky(&attempts))
            .unwrap_err();
        assert_eq!(attempts.get(), 1, "card was free: a busy device is somebody else's, no retry");
    ...
    fn a_device_that_stays_busy_after_a_takeover_says_so() {
        ...
        reserver.script(Ok(true));
        let slot = ReservationSlot::new(reserver);
        let error = open_reserved_with(Some(&slot), Some((2, "hw:2,0")), 2, Duration::ZERO, &mut || -> Result<()> {
            Err(busy())
        })
        .unwrap_err();
        assert!(format!("{error:#}").contains("does not use device reservation"), "{error:#}");
    }
}
