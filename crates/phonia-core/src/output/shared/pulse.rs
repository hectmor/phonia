...
pub fn classify(
    ...
    let kind =
        if get("device.bus") == Some("usb") || lower.contains(".usb-") || lower.contains("_usb-") {
            OutputKind::Usb
        } else if lower.contains("hdmi")
            || get("device.profile.description").is_some_and(|d| d.contains("HDMI"))
        {
            OutputKind::Hdmi
...
fn props_of(props: &protocol::Props) -> BTreeMap<String, String> {
    props
        ...
        .filter_map(|(key, value)| {
            ...
            Some((
                key.to_str().ok()?.to_string(),
                std::str::from_utf8(value).ok()?.to_string(),
            ))
        })
        .collect()
}
...
pub fn connect() -> Result<Client> {
    Client::from_env(c"phonia")
        .map_err(|error| anyhow!("no sound server (PipeWire or PulseAudio) to connect to: {error}"))
}
...
pub async fn list_outputs(client: &Client) -> Result<Vec<Output>> {
    ...
    let sinks = client
        .list_sinks()
        .await
        .context("listing the sound server's outputs")?;
    Ok(sinks
        .iter()
        .map(|sink| output_from(sink, default_name.as_deref()))
        .collect())
}
...
impl SharedVolume {
    ...
    fn channel_volume(&self, channels: u8) -> (protocol::ChannelVolume, bool) {
        ...
        for _ in 0..channels {
            channel_volume.push(protocol::Volume::from_u32_clamped(raw_from_percent(
                volume.percent,
            )));
        }
    ...
    fn attach(&self, index: u32, channels: u8) -> u64 {
        ...
        self.inner.lock().unwrap().stream = Some(AttachedStream {
            index,
            channels,
            generation,
        });
        ...
        for attempt in 0..2 {
            ...
            let sent = (|| -> Result<()> {
                ...
                protocol::write_command_message(
                    reader.get_mut(),
                    10,
                    &protocol::Command::SetSinkInputVolume(protocol::SetStreamVolumeParams {
                        index,
                ...
                protocol::write_command_message(
                    reader.get_mut(),
                    11,
                    &protocol::Command::SetSinkInputMute(protocol::SetStreamMuteParams {
                        index,
                        mute: volume.muted,
                    }),
                    *version,
    ...
    fn changed_outside(&self, index: u32, volume: Volume) {
        {
            ...
            let ours = inner
                .last_set
                .is_some_and(|at| at.elapsed() < OWN_CHANGE_ECHO);
            if ours
                || inner.stream.as_ref().map(|stream| stream.index) != Some(index)
                || inner.volume == volume
            {
                return;
            }
...
impl VolumeControl for SharedVolume {
    ...
    fn set(&self, volume: Volume) -> Result<()> {
        let mut inner = self.inner.lock().unwrap();
        inner.volume = Volume {
            percent: volume.percent.min(100),
            muted: volume.muted,
        };
...
fn find_stream(stream_id: &str) -> Result<(u32, u8)> {
    ...
    let (_, inputs) =
        protocol::read_reply_message::<protocol::SinkInputInfoList>(&mut reader, version)
    ...
    inputs
        ...
        .find(|input| {
            input
                .props
                .get_bytes(c"phonia.stream-id")
                .map(|id| id.strip_suffix(&[0]).unwrap_or(id))
                == Some(stream_id.as_bytes())
        })
...
fn read_stream_volume(index: u32) -> Result<Volume> {
    ...
    protocol::write_command_message(
        reader.get_mut(),
        4,
        &protocol::Command::GetSinkInputInfo(index),
        version,
    )?;
    ...
    let raw = channels
        .iter()
        .map(|volume| u64::from(volume.as_u32()))
        .sum::<u64>()
        / channels.len().max(1) as u64;
    Ok(Volume {
        percent: percent_from_raw(raw as u32),
        muted: info.muted,
    })
}
...
impl Transport for PulseTransport {
    fn cork(&mut self, corked: bool) -> Result<()> {
        let result = if corked {
            block_on(self.stream.cork())
        } else {
            block_on(self.stream.uncork())
        };
    ...
    fn timing(&mut self) -> Result<Timing> {
        let timing = block_on(self.stream.timing_info())
            .context("asking the server for the stream's timing")?;
        Ok(Timing {
...
impl Watcher {
    ...
    fn start(
        ...
        protocol::write_command_message(
            reader.get_mut(),
            2,
            &protocol::Command::Subscribe(
                protocol::SubscriptionMask::SINK | protocol::SubscriptionMask::SINK_INPUT,
            ),
            version,
        )?;
        protocol::read_ack_message(&mut reader)
            .context("subscribing to the sound server's outputs")?;
        let socket = reader
            .get_ref()
            .try_clone()
            .context("keeping a handle on the subscription")?;

        std::thread::Builder::new()
            .name("phonia-output-watch".into())
            .spawn(move || {
                loop {
                    match protocol::read_command_message(&mut reader, version) {
                        Ok((_, protocol::Command::SubscribeEvent(event))) => {
                            use protocol::{
                                SubscriptionEventFacility as Facility,
                                SubscriptionEventType as Kind,
                            };
                            match (event.event_facility, event.event_type) {
                                (Facility::Sink, Kind::Removed)
                                    if sink
                                        .as_ref()
                                        .is_some_and(|(index, _)| event.index == Some(*index)) =>
                                {
                                    let name = sink
                                        .as_ref()
                                ...
                                (Facility::SinkInput, Kind::Changed)
                                    if event.index == Some(stream_index) =>
                                {
                        ...
                        Err(error) => {
                            if !ring.is_closed() {
                                ring.mark_gone(&format!(
                                    "lost the connection to the sound server ({error})"
                                ));
                            }
                            return;
                        }
...
pub fn watch_outputs(on_change: impl Fn() + Send + 'static) -> Result<OutputWatch> {
    ...
    protocol::write_command_message(
        reader.get_mut(),
        2,
        &protocol::Command::Subscribe(mask),
        version,
    )?;
    ...
    std::thread::Builder::new()
        .name("phonia-outputs-watch".into())
        .spawn(move || {
            while let Ok((_, command)) = protocol::read_command_message(&mut reader, version) {
                if let protocol::Command::SubscribeEvent(event) = command {
                    use protocol::{
                        SubscriptionEventFacility as Facility, SubscriptionEventType as Kind,
                    };
...
fn raw_connect() -> Result<(BufReader<UnixStream>, u16)> {
    let path =
        pulseaudio::socket_path_from_env().ok_or_else(|| anyhow!("no sound server socket"))?;
    let mut sock =
        BufReader::new(UnixStream::connect(path).context("connecting to the sound server")?);
...
impl SharedSinkFactory {
    ...
    fn resolve(&self, client: &Client) -> Result<protocol::SinkInfo> {
        match &self.target {
            Target::Default => {
                let info = block_on(client.server_info())
                    .context("asking the sound server about itself")?;
                let name = info
                    .default_sink_name
                    .ok_or_else(|| anyhow!("the sound server has no default output"))?;
                block_on(client.sink_info_by_name(name)).context("looking up the default output")
            }
            Target::Named(name) => {
                let c_name =
                    CString::new(name.as_str()).context("an output name can't contain a NUL")?;
    ...
    fn open_stream(&self, spec: SourceSpec) -> Result<Box<dyn AudioSink>> {
        ...
        let flags = protocol::stream::StreamFlags {
            no_move: named,
            start_muted: Some(muted),
            ..Default::default()
        };
        ...
        props.set_bytes(
            c"phonia.stream-id",
            CString::new(stream_id.as_str())?.to_bytes_with_nul(),
        );
        ...
        let ring = Ring::new(
            (rate / RING_DIVISOR) as usize,
            (rate / REQUEST_DIVISOR) as usize,
        ...
        let stream = match block_on(client.create_playback_stream(params, ring.source())) {
            ...
            Err(error) => {
                ...
                return Err(anyhow!(error))
                    .context("creating the playback stream on the sound server");
            }
        };
        ...
        let watcher = match Watcher::start(
            sink,
            attached.map_or(u32::MAX, |(index, _)| index),
            ring.clone(),
            self.volume.clone(),
        ) {
        ...
        let transport = PulseTransport {
            stream,
            _watcher: watcher,
            volume: self.volume.clone(),
            generation,
        };
        Ok(Box::new(SharedSink::new(
            spec,
            ring,
            transport,
            prefix_frames,
...
mod tests {
    ...
    fn a_bluetooth_output_is_lossy_and_names_its_codec() {
        ...
        assert_eq!(
            (kind, codec.as_deref(), lossy),
            (OutputKind::Bluetooth, Some("LDAC"), true)
        );

        let (kind, codec, lossy) = classify("bluez_output.AA_BB_CC.1", &props(&[]));
        assert_eq!(
            (kind, codec, lossy),
            (OutputKind::Bluetooth, None, true),
            "recognised by name too"
        );
    }
    ...
    fn cards_are_classified_by_bus_and_name() {
        ...
        assert_eq!(
            classify("x", &props(&[("device.bus", "usb")])).0,
            OutputKind::Usb
        );
        assert_eq!(
            classify(
        ...
        assert_eq!(
            classify("phonia_spike", &props(&[])),
            (OutputKind::Other, None, false)
        );
    }
    ...
    fn the_output_list_shows_the_default_and_marks_bluetooth_as_lossy() {
        let output =
            |name: &str, description: &str, kind, codec: Option<&str>, lossy, is_default| Output {
                name: name.into(),
    ...
    fn the_target_names_the_default_or_one_output() {
        ...
        assert_eq!(
            Target::parse(Some("bluez_output.AA")),
            Target::Named("bluez_output.AA".into())
        );
    }
    ...
    fn server_properties_are_read_as_text_without_their_terminator() {
        ...
        assert_eq!(
            props_of(&p).get("media.role").map(String::as_str),
            Some("music")
        );
    }
    ...
    fn percentages_are_the_ones_the_desktop_mixers_show() {
        ...
        assert_eq!(
            raw_from_percent(200),
            65_536,
            "phonia never boosts above unity gain"
        );
        for percent in 0..=100u8 {
            assert_eq!(
                percent_from_raw(raw_from_percent(percent)),
                percent,
                "{percent}"
            );
        }
        assert_eq!(
            percent_from_raw(98_304),
            150,
            "a mixer may boost; it is reported as it is"
        );
    }
    ...
    fn an_older_stream_going_away_does_not_detach_a_newer_one() {
        ...
        assert_eq!(
            volume.get().percent,
            55,
            "the newer stream is still attached"
        );
        volume.detach(new);
    }
    ...
    impl Recorder {
        fn start(name: &str, sink: &str) -> Self {
            let path = std::env::temp_dir().join(format!(
                "phonia-shared-test-{}-{name}.raw",
                std::process::id()
            ));
            let file = std::fs::File::create(&path).unwrap();
            let child = std::process::Command::new("parec")
                .args([
                    &format!("--device={sink}.monitor"),
                    "--format=s32le",
                    "--rate=48000",
                    "--channels=2",
                    "--latency-msec=20",
    ...
    fn audio_reaches_a_named_output_complete_and_in_order_across_a_pause() {
        ...
        assert!(
            sink.delay_frames().unwrap() > 0,
            "audio is waiting while paused"
        );
        sink.resume().unwrap();
        write_all(&mut *sink, &audio[audio.len() / 2..], 2);
        sink.drain().unwrap();
        assert!(
            (sink.delay_frames().unwrap() as f64 / 48_000.0) < 0.2,
            "nothing is left after a drain"
        );
        drop(sink);
    ...
    fn the_stream_says_what_it_is_and_the_sink_is_reusable_after_a_drain() {
        ...
        sink.drain().unwrap();
        let text = std::process::Command::new("pactl")
            .args(["list", "sink-inputs"])
            .output()
            .unwrap();
        let text = String::from_utf8_lossy(&text.stdout).to_string();
        assert!(
            text.contains("media.role = \"music\"") && text.contains("resample.quality = \"10\""),
            "{text}"
        );
        ...
        assert_eq!(
            recorder.runs(),
            [(0, 24_000)],
            "the two writes are one continuous ramp"
        );
        ...
        assert!(
            reports[0].to_text().contains("SHARED (not bit-perfect)"),
            "{}",
            reports[0].to_text()
        );
    }
    ...
    fn a_named_output_that_goes_away_is_reported_as_gone_and_never_replaced() {
        ...
        assert!(
            error.downcast_ref::<crate::output::OutputGone>().is_some(),
            "{error:#}"
        );
    }
    ...
    fn a_missing_named_output_is_an_error_that_says_how_to_find_the_names() {
        ...
        let error = SharedSinkFactory::new(Target::Named("no_such_output".into()))
            .open(spec)
            .err()
            .unwrap();
    ...
    fn server_volume(sink: &str) -> Option<(String, bool)> {
        ...
        let block = text.split("Sink Input #").skip(1).find(|block| {
            block.contains("application.name = \"phonia\"")
                && block.lines().any(|line| line.trim() == wanted)
        })?;
        let volume = block
            .lines()
    ...
    fn stream_on(sink: &str) -> String {
        ...
        streams
            .lines()
            .find(|line| line.split_whitespace().nth(1) == Some(sink.as_str()))
            .unwrap()
            .split_whitespace()
            .next()
            .unwrap()
    ...
    fn a_change_made_in_the_desktops_mixer_is_followed() {
        ...
        std::process::Command::new("pactl")
            .args(["set-sink-input-volume", &index, "30%"])
            .status()
            .unwrap();
        std::process::Command::new("pactl")
